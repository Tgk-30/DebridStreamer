...
| SEC-012 | Accepted | The Android TV WebView stays on the configured server origin, disables file/content access, popup windows, and third-party cookies, and can hand only same-origin playback URLs plus a bounded short-lived bearer to Media3. Plain HTTP is accepted only for trusted LAN or encrypted tailnet use; public remote access requires HTTPS. Production APKs use one stable signing identity. | Android unit/lint gates, bridge tests, signed release job, APK clean-install verification, and the security decision check |
