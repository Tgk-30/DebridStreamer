...
fn bundled_node_path<R: Runtime>(app: &AppHandle<R>) -> Option<PathBuf> {
    ...
    let relative = match (os, arch) {
        ("macos", "aarch64") => ["darwin-arm64", "bin", "node"].as_slice(),
        ("macos", "x86_64") => ["darwin-x64", "bin", "node"].as_slice(),
        ("linux", "x86_64") => ["linux-x64", "bin", "node"].as_slice(),
        ("linux", "aarch64") => ["linux-arm64", "bin", "node"].as_slice(),
        ("windows", "x86_64") => ["win-x64", "node.exe"].as_slice(),
        ("windows", "aarch64") => ["win-arm64", "node.exe"].as_slice(),
        _ => return None,
    };
...
fn node_command<R: Runtime>(app: &AppHandle<R>) -> Command {
...
pub fn desktop_server_start<R: Runtime>(
...

    {
        ...
        *guard = Some(ServerProcess { child, port });
    }
