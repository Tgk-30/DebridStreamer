...
/// DebridStreamer patch: preload a libmpv bundled beside the wrapper (with
/// RTLD_GLOBAL) so the wrapper's own leaf-name `dlopen("libmpv.dylib"/…)`
/// resolves to this already-loaded image — which lets it run on a machine with
/// no system libmpv. Best-effort: if no bundled copy is present (e.g. dev with a
/// Homebrew/apt libmpv) the wrapper's normal lookup still runs. No-op on Windows,
/// where `libmpv-2.dll` sits next to the exe and is found there.
#[cfg(unix)]
fn preload_bundled_libmpv(lib_dir: &std::path::Path) {
    #[cfg(target_os = "macos")]
    const CANDIDATES: &[&str] = &["libmpv.dylib", "libmpv.2.dylib"];
    #[cfg(target_os = "linux")]
    const CANDIDATES: &[&str] = &["libmpv.so.2", "libmpv.so", "libmpv.so.1"];
    for name in CANDIDATES {
        ...
        if let Ok(c) = std::ffi::CString::new(path.to_string_lossy().as_bytes()) {
            ...
            if !handle.is_null() {
                info!("Preloaded bundled libmpv: {}", path.display());
                return;
            }
            warn!("Failed to preload bundled libmpv at {}", path.display());
        }
    }
}

#[cfg(not(unix))]
fn preload_bundled_libmpv(_lib_dir: &std::path::Path) {}
        ...
        self.wrapper.get_or_try_init(|| {
            ...
            if let Ok(exe_path) = std::env::current_exe() {
                if let Some(exe_dir) = exe_path.parent() {
                    ...
                    // DebridStreamer patch: Tauri bundles `resources` into the
                    // app's resource dir, NOT next to the exe, so a real bundle
                    // never finds the wrapper via the two dirs above. Derive the
                    // resource `lib` dir directly from the exe path (robust, no
                    // dependency on PathResolver::resource_dir which returns Err
                    // in some bundle contexts):
                    //   macOS:  Contents/MacOS/../Resources/lib
                    //   Linux:  <bin>/../lib/<app>/resources/lib (deb) — covered
                    //           by resource_dir() below as a fallback
                    search_dirs.push(exe_dir.join("../Resources/lib"));
                    search_dirs.push(exe_dir.join("../Resources"));
                }
            }
            // Belt-and-suspenders: also try Tauri's resolved resource dir.
            if let Ok(res_dir) = self.app.path().resource_dir() {
                search_dirs.push(res_dir.join("lib"));
            ...
            // DebridStreamer patch: the wrapper loads libmpv itself via a leaf-name
            // dlopen ("libmpv.dylib"/"libmpv.so"), which won't find a copy bundled
            // beside it and will fail on a machine with no system libmpv. Preload
            // the co-located libmpv with RTLD_GLOBAL first, so the wrapper's later
            // dlopen resolves to this already-loaded image. Best-effort — if it's
            // absent (dev with a system libmpv) the wrapper's own lookup still runs.
            if let Some(wrapper_path) = &found {
                if let Some(lib_dir) = wrapper_path.parent() {
                    preload_bundled_libmpv(lib_dir);
                }
            }

            let valid_lib_path: String = found
