...
afterEach(() => {
  delete g.__TMDB_EMBED__;
});

describe("embeddedTmdbKey", () => {
  it("returns '' when no key is embedded", () => {
    expect(embeddedTmdbKey()).toBe("");
  });

  it("deobfuscates a build-time embedded key (roundtrip)", () => {
    g.__TMDB_EMBED__ = obfuscate("abc123-XYZ_key");
    expect(embeddedTmdbKey()).toBe("abc123-XYZ_key");
  });
