...
const translateServerSubtitles = vi.fn<
  (input: { cues: SubtitleCue[]; targetLanguage: string }) => Promise<{ cues: SubtitleCue[]; providerKind: string }>
>();

vi.mock("../../lib/serverApi", () => ({
  translateServerSubtitles: (input: { cues: SubtitleCue[]; targetLanguage: string }) =>
...
describe("ServerSubtitleTranslator", () => {
  it("is always available (server-side key is managed remotely)", () => {
    expect(new ServerSubtitleTranslator().available).toBe(true);
  });

  it("passes the payload to the server API and returns translated cues", async () => {
    ...
    const translator = new ServerSubtitleTranslator();

    const out = await translator.translate(cues, "Spanish", onProgress);
  ...
  it("works without a progress callback", async () => {
    ...
    const out = await translator.translate([{ start: 0, end: 1000, text: "Hi" }], "French");
