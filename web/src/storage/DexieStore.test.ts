...
describe("library + folders", () => {
  ...
  it("sorts system folders by name when system-ness ties", async () => {
    await db.saveFolder({
      id: "sys-zeta",
      name: "Zeta",
      parentId: null,
    ...
    const folders = (await db.listFolders("favorites")).filter((folder) => folder.isSystem);
    expect(folders.map((folder) => folder.name)).toEqual(["Alpha", "Zeta"]);
  });
